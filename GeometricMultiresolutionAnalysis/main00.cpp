...
float compute_slope(unsigned long int *ScaleArray,vector<vector<double>> &Singular_MSVD, int method, unsigned long int width, unsigned long int iscales, unsigned long int p);

// Ronak added
int EstimateDimFromSpectra( unsigned long int Dims, unsigned long int nscales, double **PointsS,unsigned long int j, unsigned long int k, unsigned long int *s1,unsigned long int  width, unsigned long int DimEst);
...
int main(int argc, char **argv) {
    ...
    char a2[]="-df";
    char a3[]="/Users/ronak/Programming/CMAPA/GeometricMultiResolutionAnalysis/Sphere_528.pts";
    //char a4[]="-qf";
	...
	//
	// Eigenvalue/vector c  omputation
	//
    /*	cout << "\n Computing eigen-values/-vectors...";
     double *Evecs, *Evals;
     int nev = 30;
     
     timingList.startClock("EigComputation");
     //ComputeEig(T, nev, &Evecs, &Evals);
     timingList.endClock("EigComputation");
     
     cout << "done.";
     WriteVector(Evals, nev);*/
    
	//
	// Compute singular values at all scales
	//
	...
	if (NUM_THREADS_MSVD == 1) {                                                                                        // Single-threaded version
		...
		//if (DOWNSAMPLE_MULTISCALE_NETS) {
		//	mSVD = new MultiscaleSVD(nPts, dataPts, kdTree);
		//	mSVD->mutex_kdtree = &mutex_kdtree;
		//	mSVD->ComputeMSVDnets(&param);
        //    mSVD->PrintTimingsReport();
		//}
        //else
        ComputeMSVD(dataPts, kdTree, &param, &PointsS, &PointsV);// multi scale singular value
	} /*else {                                                                                                            // Multi-threaded version
		unsigned long i, pts_per_thread;
		pthread_t *threads = new pthread_t[NUM_THREADS_MSVD];
		param_MSVD_multi *params = new param_MSVD_multi[NUM_THREADS_MSVD];
		param_MSVD global_params;
        
		pts_per_thread = MAX(1,(unsigned long int)(ceil((double)nPts/(double)NUM_THREADS_MSVD)));                       // Decide how many pts should be assigned to each thread
        
		global_params.nPts              = nPts;
		global_params.nDims             = nDims;
		global_params.NumberOfScales    = param_NumberOfScales;
		global_params.RadiusMode        = false;
		global_params.startIdx          = 0;
		global_params.endIdx            = nPts - 1;
		global_params.spaceAllocated    = false;
        
		// Allocate memory for PointsS
		MSVD_alloc(&global_params, &PointsS, &PointsV);
        
		// Assign args to a struct and start thread
		for (i = 0; i < NUM_THREADS_MSVD; i++) {
			params[i].dataPts = dataPts;
			params[i].kdTree = kdTree;
			params[i].paramMSVD.nPts = nPts;
			params[i].paramMSVD.nDims = nDims;
			params[i].paramMSVD.NumberOfScales = param_NumberOfScales;
			params[i].paramMSVD.RadiusMode = false;
			params[i].paramMSVD.startIdx = i * pts_per_thread;
			params[i].paramMSVD.endIdx = MIN(nPts,(i+1)*pts_per_thread-1);
			params[i].paramMSVD.spaceAllocated = true;
			params[i].PointsS = PointsS + i * pts_per_thread * params[i].paramMSVD.nDims
            * params[i].paramMSVD.NumberOfScales;
			params[i].PointsV = PointsV + i * pts_per_thread * params[i].paramMSVD.nDims
            * params[i].paramMSVD.NumberOfScales;
            
			pthread_create(&threads[i], NULL, ComputeMSVD_multi, (void *) (params + i));                                // Start the threads
		}
        
		int *exit_value = new int[NUM_THREADS_MSVD];
		for (i = 0; i < NUM_THREADS_MSVD; i++) {                                                                        // Wait for all threads
			// Wait for the i-th thread to finish
			pthread_join(threads[i], (void**) (&(exit_value[i])));
			cout << "\n Thread " << i << " completed.";
		}
	...
	//if (DOWNSAMPLE_MULTISCALE_NETS) {
	//	FILE *File;
	//	File = fopen("GeometricMultiResolutionAnalysis.out", "wb");
	//	mSVD-> WriteS(File);
	//	fclose(File);
	//}
	//else
	...
	/*cout << "Point:"; WriteVector(dataPts[k - 1], nDims); WriteMatrix(PointsV, nDims, nDims);*/
...
//	delete mSVD;
	...
	//W = cs_spalloc(4, 4, 16, 1, 1);
	// Load matrix from file
	//cs_LoadFromFile(pFileName.c_str(), &A, 0, 0, 0, 1, 1);
...
int ComputeEig(cs*T, int nev, double **Evecs, double **Evals) {
	...
	*Evals = new double[nev];
    ...
    /*	// Call ARPACK
     //dsaupd(n, nev, Evals, Evecs, &s_Av);
     int ido[1]; ido[1]=0;
     char bmat[1]; bmat[1] = 'I';
     char which[3] = "LA";
     double tol = 1e-6;
     ...
     double v[n*ncv];
     int iparam[11];
     dsaupd_( ido, bmat, &n, which, &nev, &tol, resid, ncv, v, &n, iparam, <#int *ipntr#>, <#double *workd#>, <#int *iwork#>, <#double *workl#>, <#int *lworkl#>, <#int *info#>)
     */
	return 0;
}
...
int Write3MatrixToFile(double*v, unsigned long int m, unsigned long int n, unsigned long int p, const char*FileName) {
	...
	/*	unsigned long int k, j;
	 for (k = 0; k < m; k++)
	 for (j = 0; j < n; j++)
	 fwrite(v[k][j], sizeof(double), p, File);
	 */
...
int MSVD_alloc_onlyV(param_MSVD *params, double **PointsV/*,double *Multiply, double highboundry, k , double *X_C */) {
...
int MSVD_allocV(double **PointsV, unsigned long nDims, unsigned long nPts, unsigned long nPtsToCompute/*,double *Multiply, double highboundry, k , double *X_C */) {
    
		// Allocate memory for singular values and vectors
    
    PointsV= new double* [nDims];
    for (unsigned long i=0; i<nDims; i++)
        PointsV[i]= new double[nDims];
                for (unsigned long i=0; i<nDims;i++)
            for (unsigned long j=0; j<nDims; j++)
                PointsV[i][j]=0;
        
        /* how to make a matrix using arrays
         //     double **V;
         //    V=new double*[NROW];
        ...
        // ronak added:
        // *PointsV = new double[params->nDims * params->nDims]; //MM: TODO
...
//	unsigned long nPtsToCompute = params->endIdx - params->startIdx + 1;
	...
	if (!params->spaceAllocated) {
        ...
        X_C= new double* [nnpts];
        for (unsigned long i=0; i<nnpts; i++)
        ...
        Multiply= new double* [nnpts];
        for (unsigned long i=0; i<nnpts; i++)
            Multiply[i]= new double[params->nDims];
...
/////Ronak added//////////////////////////////////////////
int EstimateDimFromSpectra( unsigned long int Dims,unsigned long int nscales, double **PointsS, unsigned long int j, unsigned long int k, unsigned long int *s1, unsigned long int  width, unsigned long int DimEst, unsigned long int *Goodscales)
{ float alpha1=0.1, alpha2=0.2;
   ...
   // double Singular_MSVD[nscales]; // array of last signular values
    ...
    unsigned long int ScaleArray[nscales]; // array of scales
 ...
 Singular_MSVD[iscale][singular]= *((*PointsS) + k * nscales * Dims + iscale * Dims + singular);
   ...
   // ScaleArray: scales array
    ...
    unsigned long int l;
    i= width;
    slope = compute_slope(ScaleArray,Singular_MSVD, 2, width, i, p);
    ...
    {
    l=i;
    while ((l<jMax)&& (slope<=alpha2))
    {
        l++;
    slope= compute_slope(ScaleArray,Singular_MSVD, 2, width, l, p );
    }
...
//    float meanvalue;
    ...
    while (p>1 && (isaNoisySigularValue==true))
    {
        ...
        if (slope<=alpha2)
        {
          ...
          if (compute_Mean(ScaleArray,Singular_MSVD, width, l, p+1)> 0.2)
...
/// Ronak Added: 
  ...
  float compute_slope(unsigned long int *ScaleArray,vector<vector<double>> &Singular_MSVD, int method, unsigned long int width, unsigned long int iscales, unsigned long int p)
{
   double Multiplys1_sp[width];
    double Multiply_power_s1[width];
    ...
    float slope;
  if(Singular_MSVD.empty()) { cout << "No singular values" << endl; return 1000; };
    if (method ==2)
    {
        for (unsigned long int i=width-iscales; i<2*width-iscales; i++)
        {
            Multiplys1_sp[i]=(double)(ScaleArray[i])*(double)(Singular_MSVD[i][p]);
            sums1_sp+= Multiplys1_sp[i];
            sums1+=(double)ScaleArray[i];
            sumsp+=Singular_MSVD[i][p];
        }
        ...
        Multiply_power2= sums1*sums1;
        slope= (sums1_sp-sums1*sumsp/width)/ (sum4_power_s1 - Multiply_power2/ width); 
            }
...
{
    double MinusSp_S[width];
    double MinusS_S[width];
    double divide[width];
    double Sum=0;
    //if mean((sp(j-width+1:j)-S_MSVD(j-width+1:j,p+1))./(S_MSVD(j-width+1:j,1)-S_MSVD(j-width+1:j,p+1)))> 0.2
    //isaNoisySingularValue = false;

    for (unsigned long int i=l-width+1; i<l; i++)
    { MinusSp_S[i] =(double)ScaleArray[i]-Singular_MSVD[i][p];
    MinusS_S[i] =Singular_MSVD[i][0]-Singular_MSVD[i][p];}
    ...
    for (k = 0; k < nPtsToCompute; k++) {
       ...
       // MSVD_alloc_onlyV(params, PointsV);
            ...
            
        {
			...
			Cov_SVD(PointsForSVD, nLocalPts[k * params->NumberOfScales + j], params->nDims, (*PointsS) + k * params->NumberOfScales * params->nDims + j * params->nDims,(*PointsV));
...
delete[] PointsV;
   ...
   // EstimateDimFromSpectra(params->nDims,params->NumberOfScales,PointsS,j,k, kNNs, width, EstimateDimension, Goodscales);
    ...
    //    pthread_mutex_lock(&mutex_1);
     ...
     //   pthread_mutex_unlock(&mutex_1);
        nLocalPts[k * params->NumberOfScales + highboundry]=kNNs[highboundry];// number of points in high boundry for kth point
        //}
        
        for (p = 0; p < nLocalPts[k * params->NumberOfScales + highboundry]; p++) {                                           // Organize selected points into matrix.
            for (d = 0; d < params->nDims; d++)
            {
                PointsForMean[p * params->nDims + d] = (dataPts[nnIdx[p]])[d];
             //   cout<<"\n mean"<<"["<<k<<"]"<<"["<<highboundry<<"]:"<<PointsForMean[p * params->nDims + d];
            }
        }
        /*testing correctness
        for (p = 0; p < 2; p++) {
 ...
 double* CntrPt = new double[params->nDims];
      ...
      //  MSVD_alloc_2(params,Multiply, highboundry, k , X_C, params->nPts);
        ...
        for (unsigned long i=0; i<params->nPts; i++)
            Multiply[i]= new double[EstimateDimension];//params->nDims];
        ...
        while(imulti<params->nPts)
        {
            for (unsigned long d=0 ; d<params->nDims; d++)
            {
                Summulti += X_C[imulti][d]* PointsV[d][iVmulti];
            }//(*(*(PointsV)+iV*params->nDims+d)); //*(*(PointsV)+iV*params->nDims+d);
     ...
     /*   for (unsigned long l=0; l<params->nPts; l++)
        ...
        //cout<<"\n PowerMultiply: "<<PowerMultiply[i]<<endl;
        }
        ...
        CreatingA_matrix[i][k]=heightminus[i];
           // cout<<"\n CreatingA_matrix: "<<heightminus[i]<<endl;
  ...
  /*  for (unsigned long l=0; l<20; l++)
    {
        for (unsigned long g=0; g<3; g++)
            cout<<"\n creating matrix"<<CreatingA_matrix[l][g]<<'\t';
    cout<<endl;
    }*/
            
   // Wcomputation(CreatingA_matrix, params->nPts, nPtsToCompute, &PointsSS, &PointsU);
    double *AT_one= new double [nPtsToCompute];
    // vector<float> A_hat(Matrix[0].size());
    double *degreesMatrix= new double [params->nPts];
    
    //   double **PointsS, **PointsU;
    // *PointsS = 0;
	// *PointsU = 0;
    double *PointsSadd=0,*PointsUadd=0 ;
    double **PointsSS= &PointsSadd;
    double **PointsU=&PointsUadd;
    //MSVD_allocU(PointsS, PointsU,nPts, bags);
    *PointsSS = new double[params->nPts * nPtsToCompute];
    
    for (unsigned long int k = 0; k < params->nPts * nPtsToCompute; k++)
    {*(*PointsSS + k) = 0;
           // cout<<"taamam"<<*(*PointsSS + k);
    }
    
    PointsU= new double* [params->nPts];
    for (unsigned long i=0; i<params->nPts; i++)
        PointsU[i]= new double[params->nPts];
    
    
    for (unsigned long i=0; i<params->nPts;i++)
        for (unsigned long j=0; j<params->nPts; j++)
        { PointsU[i][j]=0;
            // cout<<"\n vector"<<PointsU[i][j]<<endl;
        }
    ...
    for (unsigned long j=0; j<nPtsToCompute;j++ )
    {
        ...
        for (unsigned long i=0; i<params->nPts; i++)
            ...
            sumsum+=CreatingA_matrix[i][j];
        AT_one[j]=sumsum;
       // cout<<"\n it is correct for computing U"<<AT_one[j]<<endl;
    ...
    {sumsum2=0;
        for (unsigned long j=0; j<nPtsToCompute; j++)
            sumsum2+=CreatingA_matrix[i][j]*AT_one[j];
        degreesMatrix[i]=sumsum2;
       // cout<<"\n it is correct for degreesMatrix: "<<degreesMatrix[i]<<endl;
    ...
    for (unsigned long i=0; i<params->nPts; i++)
    {
        degreesMatrix[i]=1/sqrt(degreesMatrix[i]);
    ...
    for (unsigned long i=0; i<params->nPts; i++)
            for (unsigned long j=0; j<nPtsToCompute; j++)
            {
        CreatingA_matrix[i][j]  = CreatingA_matrix[i][j]*degreesMatrix[i];
    
                //cout<<"\n iCreatingA_matrix: "<<CreatingA_matrix[i][j]<<endl;
            }

    
    double *pointpoint = new double [params->nPts*nPtsToCompute];
    
    for (unsigned long i=0; i<params->nPts; i++)
        for (unsigned long j=0; j<nPtsToCompute; j++)
            pointpoint[i*nPtsToCompute+j]  =  CreatingA_matrix[i][j] ;
    //////
    
    Cov_SVD_withU( pointpoint, params->nPts, nPtsToCompute, (*PointsSS),(*PointsU));
    // U is coloumns wise means each coloumns in a vector
    for (unsigned long i=0; i<KMeans; i++){
        for (unsigned long j=0; j<params->nPts; j++){
            cout<<PointsU[j][i]<<'\t';
        }
