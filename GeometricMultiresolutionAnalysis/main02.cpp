...
/*
  ...
  p=p-1;// next smallest singular value for scales
	...
	if (method ==2)
	{
        for (unsigned long int i=abs(width-iscales); i<abs(2*width-iscales); i++)
        {
...
int ComputeMSVD(ANNpointArray dataPts, ANNkd_tree *kdTree, param_MSVD *params, double **PointsS, double **PointsV) {
  ...
  for (k = 0; k < nPtsToCompute; k++) {
    ...
    //  EstimateDimFromSpectra(params->nDims,params->NumberOfScales,PointsS,j,k, kNNs, 5, EstimateDimension, Goodscales);
